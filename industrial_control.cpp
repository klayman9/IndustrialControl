...
#define MAX_ASCII_FRAME_SIZE 45 /* ������������ ����� ASCII-������ ������� ������� � ������ '\0'  */
#define MAX_FRAME_SIZE 21 /* ������������ ����� ������ �������� ��������� ���� */
...
static QString _ERROR_TEXT=QString::fromLocal8Bit("������:");
static QString _EXIT_TEXT=QString::fromLocal8Bit("���������");
const char *_PLC_EXCHANGE_TXT= "�����_TPM101>";
static QPalette pal_0;                                         //�������
...
{char str[256];
 ...
 if((rs232 != NULL) &&  (rs232 != INVALID_HANDLE_VALUE)) //���� ���� ���������-����� �����
    if (!CloseHandle(rs232))                             //���� �� ������
        {err_code= GetLastError();
         sprintf(str,"Kernel Fault:Unable close port handle. ��������:( %x)",  err_code);
...
int industrial_control::rs232_init(QString port_name)
{
 unsigned int err_code;
 char str[256];
 COMMTIMEOUTS CommTimeouts; // ���������� ���������� ���������, ���: LPCOMMTIMEOUTS ��� ���������� �� WINAPI
 DCB PortDCB;               // ���������� ���������� ���������, ���: LPDCB ��� ������ �������� �� WINAPI
 ...
 // ���� ���� �� ������� ����� �� ������
  if ( rs232 == INVALID_HANDLE_VALUE )
    {err_code= GetLastError();
     sprintf(str,"�������� ����� com_%d ���������. ��������:( %x)", ui->rs232_ch->value(), err_code);
     QMessageBox::critical( 0, _ERROR_TEXT, QString::fromLocal8Bit(str), _EXIT_TEXT );
     return 0;
    }
 // ��������� ��������� DCB ���: LPDCB ��� ������ �������� �� WINAPI
  PortDCB.DCBlength = sizeof (DCB);     // ������ ���������(DCB)
...
{unsigned long n;
 ...
 if(err)
    {return 0;}
 ReadFile (rs232, rx_buf, nbytes, &n, 0);  /* ������ ������ ��������� � ��� ����� */
 if(!n)
    {char str[256];
     sprintf(str, "%s schedule=0x%x: ��� ������ �� �����������.", _PLC_EXCHANGE_TXT, schedule_tm);
     log_add_str(QString::fromLocal8Bit(str));
...
/*=====�RC============================================================================================================*/
unsigned short industrial_control::owenCRC16(unsigned char* packet, int length) /* ������ ����������� ����� */
{int i, j;
 unsigned short crc;
 if(packet == NULL)
 	return 0;
...
{if(!ui->heater_manual->isChecked())           /* ���� ������������� ����������� */
    {QMessageBox::critical( 0, _ERROR_TEXT, QString::fromLocal8Bit("������� �������� �������� ��������� ������ � ������ ������ �������������"), _EXIT_TEXT );}
 schedule_tm= 0x60;
 return;
}
     ...
     case  0x1:  /*��������� ���������� �������� ������� �������� */
				...
				if(!unpackFloat24(pf.data, pf.dataSize, &v))
					 ...
					 break;
                                ...
                                ui->lcd_t->display(QString::fromLocal8Bit(str)); //������� � ���������
...
int industrial_control::profile0(void)
{
    ...
    ui->labe->setText(QString::fromLocal8Bit("����� ������: ") + difftime.toString("mm:ss"));
 return 1;
}
int industrial_control::profile1(void)
{
    ui->label->setText(QString::fromLocal8Bit("����������� �������: 1"));
...
{
    ui->label->setText(QString::fromLocal8Bit("����������� �������: 2"));
...
{
    ui->label->setText(QString::fromLocal8Bit("����������� �������: 3"));
...
{
    ui->label->setText(QString::fromLocal8Bit("����������� �������: 4"));
...
{
    ui->label->setText(QString::fromLocal8Bit("����������� �������: 5"));
...
{
    ui->label->setText(QString::fromLocal8Bit("����������� �������: 6"));
...
{
    record_count++;
    ui->labe_3->setText(QString::fromLocal8Bit("���-�� �������: ")+QString::number(record_count));
    char stroka[256];
    QString qs;
    ...
    if (record_count==record_max)
       {time8->stop();
        ui->labe_3->setText(QString::fromLocal8Bit("������ ���������"));
       }
 return 1;
}
...
{
     qDebug() << QString::fromUtf8(QString("������").toLocal8Bit());
    ...
    if ((ui->vremzap->value()!=0) && (ui->kolzap->value() !=0)) {
        ...
        QString path = QFileDialog::getSaveFileName(this, QString::fromLocal8Bit("���������"), home,"data file (*.txt)");
    ...
    log_add_str(QString::fromLocal8Bit("�������� �������������"));
    plot->reset_diagram();
    plot->view_desired(ui->desired_t->value());
